...
class SimulatorController extends Controller
{
    ...
    public function parking_form($slug)
    {
        ...
        switch(true) {
            ...
            case(isset($_POST['green'])):
                return $this->positiveValidation();
                break;
    ...
    private function parkingEntrance() {

        if($this->spots_taken < $this->spots_total || $this->spots_reserved > 0) {

            // PROVJERA postoji li rezervacija (ako postoji briše se nakon unosa)

            $code = $_POST['screen'];

            $res_exists = Reservation::where('code', $code)->where('parking_id', $this->parking->id)->first();
            ...
            if($res_exists && strlen((string) $code) == 4) {
                ...
                session()->flash('info', 'New ticket created in ' . $this->parking->city . ' (' . $this->parking->name . ')!');
                return redirect()->route('parking_select', ['slug' => $this->slug])->with(['got_ticket' => $got_ticket, 'value' => $value]);
        ...
        return redirect()->route('parking_select', ['slug' => $this->slug]);
    }

    private function createTicket() {

        if($this->spots_taken < $this->spots_total) {
    ...
    private function positiveValidation() {
        ...
        return redirect()->route('parking_select', ['slug' => $this->slug])->with('got_ticket', $got_ticket);
    }

    private function parkingLot() {

        if($this->spots_current > 0) {
    ...
    private function ticketSubmit() {
        $got_ticket = 3;
        $ticket_check = $_POST['insert_ticket'];
        $my_ticket = Ticket::where('code', $ticket_check)->where('parking_id', $this->parking->id)->first();
        ...
        if($ticket_check && $is_paid == false) {
            ...
            $working = explode('-', $this->parking->working_time);
            ...
            // CIJENA KARTE
            $price_hour = $this->parking->price_per_hour;
    ...
    private function insertCoins() {
        ...
        if(is_numeric($coin_check)) {
            ...
            return redirect()->route('parking_select', ['slug' => $this->slug])->with(['got_ticket' => $got_ticket, 'refund' => $refund]);
        } else {
    ...
    private function exitValidation() {
        ...
        if($exit_ticket_check && $is_paid == true && $penalty > 0) {
            ...
            return redirect()->route('parking_select', ['slug' => $this->slug])->with(['got_ticket' => $got_ticket, 'exit_ticket_check' => $exit_ticket_check]);
    ...
    private function exitSuccess() {
        ...
        session()->flash('info', 'You have left \'' . $this->parking->name . '\' (' . $this->parking->city . ') parking lot!');
